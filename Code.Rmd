---
title: "Seasonal Vapor Pressure Deficit and Temperature Effects on Carbon Dioxide and Water Dynamics in a Prevalent Crop Rotation in the Northern Great Plains"
...
f1a <- ggplot(Monthly_Summary) +
  ...
  geom_rect(aes(xmin = as.Date("2017-10-01"),xmax = as.Date("2018-9-01"),ymin = -Inf, ymax = Inf), fill= "#deb3f5", alpha = .002) +
  ...
  geom_rect(aes(xmin = as.Date("2020-09-01"),xmax = as.Date("2021-09-01"),ymin = -Inf, ymax = Inf), fill= "#deb3f5", alpha = .002)+
...
Monthly_filtered <- Monthly_Summary %>% 
  mutate(SWC_50 = case_when(Month == "1" ~ NA,
                            Month == "2" ~ NA,
                            Month == "3" ~ NA,
                            TRUE ~ SWC_50))
...
f1c <- ggplot(Monthly_filtered) + 
  ...
  geom_rect(aes(xmin = as.Date("2017-10-01"),xmax = as.Date("2018-9-01"),ymin = 10, ymax = Inf), fill= "#deb3f5", alpha = .002)+
  ...
  geom_rect(aes(xmin = as.Date("2020-09-01"),xmax = as.Date("2021-09-01"),ymin = 10, ymax = Inf), fill= "#deb3f5", alpha = .002)+
...
f4a <- Seasonal_Summary %>%
  filter(Season != "Full Year") %>% 
ggplot(aes(x = VPD_mean, y = ET_mean, color = Season)) +
  geom_point(aes(x = VPD_mean, y = ET_mean, shape = H_Year), size = 2) +
  geom_errorbar(aes(xmin = VPD_mean - VPDSE, xmax = VPD_mean +  VPDSE)) +
  geom_errorbar(aes(ymin = ET_mean - ETSE, ymax = ET_mean +  ETSE)) +
    scale_shape_manual(values= shapes, breaks = c("Year1", "Year2", "Year3", "Year4", "Year5", "Year6")) +
    #stat_poly_line(formula = formula, alpha = 0.2) +
    # stat_poly_eq(aes(label =  paste(after_stat(eq.label), "*\": \"*", 
    #                               after_stat(rr.label), "*\", \"*", 
    ...
    #                               sep = "")),
    #            formula = formula, size = eq_size, vstep = 0.06) +
  stat_correlation(mapping = use_label(c("R", "p.value")), label.x = "left", label.y = "top", size = eq_size) +
  theme_classic() +
  theme(legend.position="none")  +
  xlab(expression(~VPD~(hPa))) +
...
f4b <- Seasonal_Summary %>%
  filter(Season != "Full Year") %>% 
ggplot(aes(x = Tair, y = ET_mean, color = Season)) +
  geom_point(aes(x = Tair, y = ET_mean, shape = H_Year), size = 2) +
...
f4c <- Seasonal_Summary %>%
  filter(Season != "Full Year") %>% 
ggplot(aes(x = RH_ave, y = ET_mean, color = Season)) +
  geom_point(aes(x = RH_ave, y = ET_mean, shape = H_Year), size = 2) +
  ...
  theme_classic() +
    ...
    stat_correlation(mapping = use_label(c("R", "p.value")), label.x = "right", label.y = "top", size = eq_size) +
  theme(legend.position="none")  +
...
sf2a <- Seasonal_Summary %>%
  filter(Season != "Full Year") %>% 
ggplot(aes(y = -NEE, x = Tair, color = Season)) +
  geom_point(aes(y = -NEE, x =Tair, shape = H_Year), size = 2) +
  geom_errorbar(aes(ymin = -NEE - NEESE, ymax = -NEE +  -NEESE)) +
  geom_errorbar(aes(xmin = Tair - TairSE, xmax = Tair +  TairSE)) +
    scale_shape_manual(values= shapes, breaks = c("Year1", "Year2", "Year3", "Year4", "Year5", "Year6")) +
    #stat_poly_line(formula = formula, alpha = 0.2) +
...
sf2a <-  ggpar(sf2a, palette = c(#"#178df5",  
  "#8b4513", "#228b22")) #+
  #              stat_function(fun = equation, aes(x = VPD_mean), color = "#178df5", alpha = 0.5) +
  # geom_text(aes(4,65, label=(paste(expression("y = 85.7 + 20.2x")))),parse = F, color = "#178df5", size = eq_size)  +
  # geom_text(aes(3,50, label=(paste(expression("R"^2*" = 0.86")))),parse = TRUE, color = "#178df5", size = eq_size) +
...
sf2b <- Seasonal_Summary %>%
  filter(Season != "Full Year") %>% 
ggplot(aes(y = -NEE, x = RH_ave, color = Season)) +
  geom_point(aes(y = -NEE, x =RH_ave, shape = H_Year), size = 2) +
  geom_errorbar(aes(ymin = -NEE - NEESE, ymax = -NEE +  -NEESE)) +
    geom_errorbar(aes(xmin = RH_ave - RHSE, xmax = RH_ave +  RHSE)) +
    scale_shape_manual(values= shapes, breaks = c("Year1", "Year2", "Year3", "Year4", "Year5", "Year6")) +
    #stat_poly_line(formula = formula, alpha = 0.2) +
...
sf2b <-  ggpar(sf2b, palette = c(#"#178df5",  
  "#8b4513", "#228b22")) #+
  #              stat_function(fun = equation, aes(x = VPD_mean), color = "#178df5", alpha = 0.5) +
  # geom_text(aes(4,65, label=(paste(expression("y = 85.7 + 20.2x")))),parse = F, color = "#178df5", size = eq_size)  +
  # geom_text(aes(3,50, label=(paste(expression("R"^2*" = 0.86")))),parse = TRUE, color = "#178df5", size = eq_size) +
...
sf2 <- plot_grid(sf2a, sf2b,  labels = "AUTO", ncol = 2, nrow = 1, common.legend = TRUE)

sf2


ggsave(here("Graphs", "sf2.pdf"), height = 3)
# 
ggsave(here("Graphs", "sf2.svg"), dpi = 300, height = 3)
